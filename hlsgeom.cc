...
istream &operator>>(istream &is, i128 &i) {
  i64 _i;
  is >> _i;
  i = _i;
  return is;
}
ostream &operator<<(ostream &os, const i128 &i) {
  if (i == 0) {
    return os << "0";
  }
  string s;
...
int main() {
  ...
  auto f = [&](auto &f, int l, int r) -> Convex {
    int m = (l + r) / 2;
    if (l + 1 == r) {
      return convexhull({Point(0, 0), p[m]});
    }
